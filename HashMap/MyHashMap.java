...
public class MyHashMap<K extends Comparable<K>, V> implements MyMap<K, V> {
    ...
    @Override
    public V get(K key) {
        int tableIdx = (key.hashCode() % table.length);
        if (table[tableIdx] == null) {
            return null;
        }
        else {
            Entry<K, V> curr = table[tableIdx];
            while (curr != null) {
    ...
    public V put(K key, V value) {
        ...
        if(table[tableIdx] != null){
            ...
            while (curr != null) {
                if (curr.key.equals(key)) {
                    old = curr.value;
                    curr.value = newKeyVal.value;
                    return old;
                }
                curr = curr.next;
            }
            newKeyVal.next = table[tableIdx];
            table[tableIdx] = newKeyVal;
            numEntries++;
        }
        else if (table[tableIdx] == null) {
            table[tableIdx] = newKeyVal;
            numEntries++;
            return old;
        }
        if((getLoadFactor() > MAX_LOAD_FACTOR) && primeIndex < 11){
            rehash();
        }
        return old;
    }
    ...
    private void rehash() {
            ...
            for (int i = 0; i < table.length; i++) {
                ...
                while (curr != null) {
                    int tableIdx = (curr.key.hashCode() % newTable.length);
                    Entry<K, V> newVal = new Entry<>(curr.key, curr.value);
                    if(newTable[tableIdx] == null){
                        newTable[tableIdx] = newVal;
                    }
                    else{
                        newVal.next = newTable[tableIdx];
                        newTable[tableIdx] = newVal;
                    }
                    curr = curr.next;
                }
